...
async def get_contact_by_id(contact_id: int, user: User, db: Session):
    contacts = db.query(Contact).filter(and_(Contact.id == contact_id, Contact.user_id == user.id)).first()
    return contacts

async def get_contact_by_email(email: str, user: User, db: Session):
    contacts = db.query(Contact).filter(and_(Contact.email == email, Contact.user_id == user.id)).first()
    return contacts

async def search_contact(query: str, user: User, db: Session):
    contact = db.query(Contact).filter(and_(Contact.firstname.ilike(f'%{query}%'), Contact.user_id == user.id)).all()
    if contact:
        return contact
    contact = db.query(Contact).filter(and_(Contact.lastname.ilike(f'%{query}%'), Contact.user_id == user.id)).all()
    if contact:
        return contact
    contact = db.query(Contact).filter(and_(Contact.email.ilike(f'%{query}%'), Contact.user_id == user.id)).all()
    if contact:
        return contact
...
async def get_birthday_per_week(user: User, db: Session):
    ...
    for contact in all_contacts:
        if timedelta(0) <= (datetime.now().date() - (contact.birthday.replace(year=int((datetime.now()).year)))) <= timedelta(7):
